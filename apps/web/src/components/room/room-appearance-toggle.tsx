...
export function RoomAppearanceToggle({
  ...
  return (
    <div
      ...
      className={cn("inline-flex p-0.5", ROOM_HEADER_CONTROL, className)}
    >
      <PostItIconTooltip label="Light mode" side="bottom">
        <Button
          ...
          size="sm"
          className={cn("h-8 w-8 p-0", appearance !== "light" && "text-foreground")}
      ...
      <PostItIconTooltip label="Dark mode" side="bottom">
        <Button
          ...
          size="sm"
          className={cn("h-8 w-8 p-0", appearance !== "dark" && "text-foreground")}
