-- Private room access after leaving:
-- - Inactive sweep removes room_members only; approved access stays → user can rejoin.
-- - Owner kick sets access to revoked → user cannot rejoin or see room in private tab.

ALTER TABLE public.room_access_requests
  DROP CONSTRAINT IF EXISTS room_access_requests_status_check;

ALTER TABLE public.room_access_requests
  ADD CONSTRAINT room_access_requests_status_check
