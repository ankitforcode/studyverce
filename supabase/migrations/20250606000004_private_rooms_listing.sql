-- Private rooms tab: owned, invited (member), or pending access.
-- Friends tab: placeholder until friendship-based listings ship.
...
CREATE OR REPLACE FUNCTION public.get_user_friend_rooms()
RETURNS JSON
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
AS $$
BEGIN
  -- Friendship-based room listings will be wired here later.
  RETURN '[]'::JSON;
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_user_friend_rooms() TO authenticated;
