-- Friends tab: include the current user's own private rooms (owners could not find them).
...
BEGIN
  ...
  FROM (
    ...
    WHERE
      (sr.owner_id = v_user_id AND sr.is_public = FALSE)
      OR (
        sr.is_public = FALSE
        AND sr.owner_id <> v_user_id
        AND (rm.user_id IS NOT NULL OR rar.id IS NOT NULL)
      )
  ) sub;
